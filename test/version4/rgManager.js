...
const RGManager = artifacts.require('./RGManagerPrototype_v4.sol');
const RGAssetOwnershipCardCloneFactory = artifacts.require('./RGAssetOwnershipCardCloneFactory.sol');
const RGAssetOwnershipCard = artifacts.require('./RGAssetOwnershipCardPrototype_v4.sol');
const RGTransactionRules = artifacts.require('./RGTransactionRulesPrototype_v4.sol');
const ownedBase = require('../ownedBase');
const deployHelperContracts = require('../helpers/deployHelperContracts');

contract('RGManager v4', function(accounts) {
  ...
  it('should be possible to start redemption and send full bar coins to invoice', function() {
    ...
    .then(result => {
      ...
      invoiceAddress = result.logs[0].args.to;
  ...
  it('should be possible to start redemption and send full bar coins to invoice right after AC deploying', function() {
    const user = accounts[1];
    const invoice = 'invoice1';
    const coins = 1000;
    ...
    .then(result => RGAssetOwnershipCard.at(result.logs[1].args.rgAssetOwnershipCardAddress))
    .then(instance => RGAC1 = instance)
    .then(() => RGAC1.transferToInvoice.call(invoice, {from: rgOwner}))
    .then(assert.isTrue)
    .then(() => RGAC1.transferToInvoice(invoice, {from: rgOwner}))
    .then(result => {
      assert.equal(result.logs.length, 2);
      assert.equal(result.logs[0].event, 'Transfer');
      ...
      assert.equal(result.logs[1].event, 'Transfer');
      assert.equal(result.logs[1].args.from, rgOwner);
      assert.equal(web3.isAddress(result.logs[1].args.to), true);
      assert.equal(result.logs[1].args.value, coins);
    })
    .then(() => assertBalance(rGManagerClone, rgOwner, 0))
    .then(() => assertBalance(RGAC1, rgOwner, 0))
    .then(() => assertBalance(rGManagerClone, invoiceAddress, 1000))
    .then(() => assertBalance(RGAC1, invoiceAddress, 1000))
  ...
  it('should be posibble to send coins via spend function', function() {
    const recipient = accounts[1];
