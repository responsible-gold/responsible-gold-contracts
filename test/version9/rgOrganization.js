...
const RGManager = artifacts.require('./RGManagerPrototype_v9.sol');
const RGAssetOwnershipCardCloneFactory = artifacts.require('./RGAssetOwnershipCardCloneFactory.sol');
const RGAssetOwnershipCard = artifacts.require('./RGAssetOwnershipCardPrototype_v9.sol');
const RGTransactionRules = artifacts.require('./RGTransactionRulesPrototype_v9.sol');
const RGRuleAuthorizer = artifacts.require('./RGRuleAuthorizerPrototype_v9.sol');
const RGValidator = artifacts.require('./RGValidatorPrototype_v9.sol');
...
contract('RGOrganization v9', function(accounts) {
  ...
  it('should NOT be possible to constructOrganization more than 1 time with same params', function() {
    let organizationAddress;
    let organization;
    ...
    .then(() => RGOrganization.at(organizationAddress))
    .then(instance => organization = instance)
    .then(() => organization.constructOrganization(rgOrganizationFactory.address, rgRuleAuthorizerClone.address, rgOwner))
    .then(result => {
      assert.equal(bytesToString(result.logs[0].args.error), 'Organization already constructed');
    })
  });

  it('should NOT be possible to constructOrganization more than 1 time with new org user', function() {
    let organizationAddress;
    let organization;
    ...
    .then(() => RGOrganization.at(organizationAddress))
    .then(instance => organization = instance)
    .then(() => organization.constructOrganization(accounts[2], accounts[2], accounts[2]))
    .then(result => {
      assert.equal(bytesToString(result.logs[0].args.error), 'Organization already constructed');
    })
    .then(() => organization.companyUsers(accounts[2]))
    .then(assert.isFalse);
  });

  it('should set rg organization factory after organization creation', function() {
    let organizationAddress;
    let organization;
  ...
  it('should be possible to make redemptionTransferToInvoice with 3 cosigners via validator contract', function() {
    let organizationAddress;
    let organization;
    let acc1;
    let account1;
    let acc2;
    let data;
    let redemptionTransferToInvoice;
    let forwardData;
    let hashedForwardData;
    let confirmData;
    let confirmData2;
    ...
    const nonce2 = 2;
    const nonce3 = 3;
    const tokensToInvoice = 100;
    const invoice = 'invoice1';
    const invoiceAddress = util.addHexPrefix(web3.sha3(invoice).slice(-40));
    ...
    .then(() => rGManagerClone.transfer(acc1, 400, {from: rgOwner}))
    .then(() => assertBalance(rGManagerClone, acc1, 400))
    .then(() => assertBalance(rGManagerClone, invoiceAddress, 0))
    .then(() => data = organization.contract.createAccount.getData())
    .then(() => rgRuleAuthorizerClone.confirmByAuthorizer(web3.sha3(data, {encoding: 'hex'}), organization.address, rgOwnerBytes32, 1, 1, {from: ruleAuthorizer}))
    .then(() => organization.createAccount())
    .then(result => {
    ...
    .then(() => redemptionTransferToInvoice = rGManagerClone.contract.redemptionTransferToInvoice.getData(invoice, 100))
    .then(() => forwardData = organization.contract.forward.getData(acc1, rGManagerClone.address, 0, redemptionTransferToInvoice, false))
    .then(() => hashedForwardData = web3.sha3(forwardData, {encoding: 'hex'}))
    .then(() => authorizerSig = prepareSignature(ruleAuthorizerPK, hashedForwardData, organization.address, rgProxyAddress, operationId, requiredConsumptions3, rgRuleAuthorizerClone.address, nonce, rgValidatorClone.address))
    .then(() => confirmData = rgRuleAuthorizerClone.contract.confirm.getData(hashedForwardData, organization.address, addLeftToAddressWithPrefix(rgProxyAddress), operationId, requiredConsumptions3, nonce, authorizerSig.v, util.bufferToHex(authorizerSig.r), util.bufferToHex(authorizerSig.s)))
    .then(() => userForwardData = rgUser.contract.forward.getData(organization.address, 0, forwardData))
    ...
    .then(() => rgTransactionRulesClone.removeFromWhitelist(rgOwner))
    .then(() => rgValidatorClone.forwardCalls([rgRuleAuthorizerClone.address, rgRuleAuthorizerClone.address, rgRuleAuthorizerClone.address, rgUser.address, rgUser2.address, rgUser3.address, '0x0', '0x0', '0x0', '0x0', '0x0'], confirmData, confirmData2, confirmData3, userForwardData, userForwardData2, userForwardData3, '0x1', '0x1', '0x1', '0x1', '0x1'))
    .then(() => assertBalance(rGManagerClone, acc1, 300))
    .then(() => assertBalance(rGManagerClone, invoiceAddress, tokensToInvoice));
  })
  ...
  it('should throw an error if call to external contract failed from user contract', function(){
    let rgProxyAddress;
    let rgUserAddress;
    let rgUser;
    let data;
    const userContractOwner = accounts[1];

    return rgUserClonePool.deploy()
    .then(result => {
      rgProxyAddress = result.logs[0].args.rgProxyAddress;
      rgUserAddress = result.logs[0].args.contractAddress;
    })
    .then(() => RGUser.at(rgUserAddress))
    .then(instance => rgUser = instance)
    .then(() => rgUserClonePool.assignTo(rgUser.address, userContractOwner, false))
    .then(() => rGManagerClone.deploy('chip1', 1000, rgOwner))
    .then(() => rGManagerClone.transfer(rgProxyAddress, 400, {from: rgOwner}))
    .then(() => assertBalance(rGManagerClone, rgProxyAddress, 400))
    .then(() => rgTransactionRulesClone.addToWhitelist(userContractOwner))
    .then(() => data = rGManagerClone.contract.transfer.getData(rgOwner, 100))
    .then(() => rgUser.forward(rGManagerClone.address, 0, data, {from: userContractOwner, gas: 100000}))
    .then(result => {
      assert.equal(bytesToString(result.logs[0].args.message), 'External call failed');
    })
    .then(() => assertBalance(rGManagerClone, rgProxyAddress, 400))
  });

  it('should not allow to call initInsecure for contract owner more than 1 time', function(){
    let rgProxyAddress;
    let rgUserAddress;
    let rgUser;
    const userContractOwner = accounts[1];
    ...
    .then(instance => rgUser = instance)
    .then(() => rgUserClonePool.assignTo(rgUser.address, userContractOwner, false))
    .then(() => asserts.throws(rgUser.initInsecure(accounts[2], {from: userContractOwner})))
  });

  it('should not allow to call initInsecure 1st time for not contract owner', function(){
    let rgProxyAddress;
    let rgUserAddress;
    let rgUser;
    const userContractOwner = accounts[1];
    ...
    .then(() => RGUser.at(rgUserAddress))
    .then(instance => rgUser = instance)
    .then(() => rgUser.initInsecure(accounts[2], {from: userContractOwner}))
    .then(result => {
      assert.equal(bytesToString(result.logs[0].args.message), 'Access denied');
    })
    .then(() => rgUser.doNotAlwaysRequireCosignature())
    .then(assert.isFalse)
    .then(() => rgUser.contractOwner())
    .then(result => assert.equal(result, rgUserClonePool.address));
  });
