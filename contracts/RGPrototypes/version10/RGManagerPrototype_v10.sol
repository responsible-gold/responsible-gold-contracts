...
import {RGAssetOwnershipCardPrototype_v10 as RGAssetOwnershipCard} from './RGAssetOwnershipCardPrototype_v10.sol';
import '../../OwnedPrototype.sol';
import {ERC20Interface as ERC20} from '../../Interfaces/ERC20Interface.sol';
import '../../Interfaces/RGManagerInterface.sol';
 ...
 * didn't happen yet.
 */
contract RGManagerPrototype_v10 is OwnedPrototype, ERC20, RGManagerInterface {
    ...
    mapping(uint => RGAssetOwnershipCardInterface) public rgAssetOwnershipCards;
    ...
    mapping(address => RGAssetOwnershipCardInterface[]) public userRGAssetOwnershipCards;
    ...
    //RG fees
    mapping(address => uint) public lastTimeFeeToCollect;
    mapping(address => uint) public feeToCollect;
    uint public defaultFeeTime;
    uint constant public divisibilityFactor = 10**4;
    uint public yearlyFee;
    address public feeCollector;
    mapping(address => bool) public nonFeeAddresses;

    //RG registry
    RGRegistryInterface public registry;
    ...
    event DefaultFeeTimeSet(uint time);
    event FeeToCollectUpdated(address payer, uint addedFee, uint totalFee, uint totalDays, uint balance);
    event FeePayed(address payer, uint fee);
    event FeeCollectorSet(address feeCollectorAddress);
    event FeeSet(uint yearly);
    event LastTimeFeeToCollectUpdated(address payer, uint lastTimeFee);
    event AvailableBalance(uint availableBalance, address balanceAddress);
    event NonFeeAddressAdded(address nonFeeAddress);
    event NonFeeAddressRemoved(address nonFeeAddress);
    event TransactionRulesSet(address rulesAddress);
    event CloneFactorySet(address cloneFactoryAddress);
    event RegistrySet(address registryAddress);
    event MerchantTransfer(bytes32 icap, address from, address to, uint value);
    ...
    function RGManagerPrototype_v10() {
        constructRGManager(0x1, 0, RGAssetOwnershipCardCloneFactoryInterface(0x0), RGTransactionRulesInterface(0x0), RGRegistryInterface(0x0));
    }
     ...
     *
     */
    function constructRGManager(address _owner, uint8 _baseUnit, RGAssetOwnershipCardCloneFactoryInterface _cloneFactory, RGTransactionRulesInterface _transactionRules, RGRegistryInterface _registry) returns(bool) {
    ...
    function setRGTransactionRules(RGTransactionRulesInterface _transactionRules) onlyOwner() returns(bool) {
        ...
        transactionRules = _transactionRules;
        TransactionRulesSet(_transactionRules);
        return true;
    }

    function setRGAssetOwnershipCardCloneFactory(RGAssetOwnershipCardCloneFactoryInterface _cloneFactory) onlyOwner() returns(bool) {
        if (address(_cloneFactory) == 0x0) {
            Error('Clone factory is not valid');
            return false;
        }

        cloneFactory = _cloneFactory;
        CloneFactorySet(_cloneFactory);
        return true;
    }

    function setRGRegistry(RGRegistryInterface _registry) onlyOwner() returns(bool) {
        if (address(_registry) == 0x0) {
            Error('Registry address is not valid');
            return false;
        }
    ...
    function _deploy(string _chip, uint _totalCoins, address _coinsOwner) internal {
        RGAssetOwnershipCardInterface rgAssetOwnershipCard = _deployRGAssetCard(_chip, _totalCoins, _coinsOwner);
    ...
    function _deployRGAssetCard(string _chip, uint _totalCoins, address _coinsOwner) internal returns(RGAssetOwnershipCardInterface) {
        RGAssetOwnershipCardInterface rgAssetOwnershipCard = cloneFactory.deploy();
    ...
    function mintGcoins(string _chip, address _gCoinsOwner) onlyOwner() notUnderMigration() returns(bool) {
        ...
        RGAssetOwnershipCardInterface rgAssetOwnershipCard = RGAssetOwnershipCardInterface(rgAssetOwnershipCardAddress);
    ...
    function _mint(RGAssetOwnershipCardInterface rgAssetOwnershipCard, address _gCoinsOwner, uint coinsToMint) internal {
    ...
    function availableBalance(address _owner) constant returns(uint) {
        uint calculatedFee;
        uint calculatedDays;

        (calculatedFee, calculatedDays) = _calculateFee(_owner);
        return balances[_owner] - feeToCollect[_owner] - calculatedFee;
    }

    /**
     * Gets Gcoins user balance, fee and available balance
     *
     * @param _owner user address
     *
     * @return ownerBalance, ownerFee, ownerAvailableBalance.
     */
    function getUserData(address _owner) constant returns(uint ownerBalance, uint ownerFee, uint ownerAvailableBalance) {
        return (balanceOf(_owner), balanceOf(_owner) - availableBalance(_owner), availableBalance(_owner));
    }
    ...
    /**
     * Transfers user balance from the caller to specified ICAP address.
     *
     * @param _icap address to give to.
     * @param _value amount to transfer.
     *
     * @return success.
     */
    function transferToMerchant(bytes32 _icap, uint _value) returns(bool success) {
        if (address(registry) == 0x0) {
            Error('Set registry address first');
            return false;
        }
        ...
        (to, asset, parsed) = registry.parse(_icap);
        ...
        if (!spend(to, _value, 0, '')) {
            Error('Failed to transfer to merchant');
            return false;
        }
        MerchantTransfer(_icap, msg.sender, to, _value);
        return true;
    }
    ...
    function _transfer(address _from, address _to, uint _value) internal returns(bool success) {
        ...
        //calculating the fee on both party
        _updateFee(_to);
        _updateFee(_from);

        if (availableBalance(_from) < _value) {
            Error('Not possible to cover fee');
            AvailableBalance(availableBalance(_from), _from);
            return false;
        }

        _transferACcoins(_from, _to, _value);
        _transferBalances(_from, _to, _value);
        return true;
    }

    function _updateFee(address _payer) internal {
        uint addedFee;
        uint daysToCollect;

        //setting for the 1st time if unset
        if (lastTimeFeeToCollect[_payer] == 0) {
            lastTimeFeeToCollect[_payer] = defaultFeeTime;
        }

        (addedFee, daysToCollect) = _calculateFee(_payer);

        //update fee for regular user
        _updateUserFee(_payer, addedFee, daysToCollect);
    }

    function _calculateFee(address _payer) internal constant returns(uint, uint) {
        uint lastTimeCollectFee = lastTimeFeeToCollect[_payer];

        if (!_isValidPayer(_payer)) {
            return (0, 0);
        }

        if (lastTimeCollectFee == 0) {
            lastTimeCollectFee = defaultFeeTime;
        }

        uint daysToCollect = (now - lastTimeCollectFee) / 1 days;

        if (daysToCollect == 0) {
            return (0, 0);
        }

        uint addedFee = _calculateTotalDaysFee(daysToCollect, _payer);

        return (addedFee, daysToCollect);
    }

    function _isValidPayer(address _payer) internal constant returns(bool) {
        if (defaultFeeTime == 0 || defaultFeeTime > now) {
            return false;
        }

        if (nonFeeAddresses[_payer]) {
            return false;
        }

        if (_isInvoice(_payer)) {
            return false;
        }
        return true;
    }

    function _calculateTotalDaysFee(uint _daysToCollect, address _payer) internal constant returns(uint) {
        uint balance = balances[_payer];
        uint tempFeeToCollect = feeToCollect[_payer];
        uint tempYearlyFee;

        //do not calculate fee if address-receiver has no coins yet
        if (balance == 0) {
            return 0;
        }

        for (var i = _daysToCollect; i > 0; i--) {
            tempYearlyFee = yearlyFee * (balance - tempFeeToCollect);

            //finish calculation if payer doesn't have balance minus feeToCollect
            if (tempYearlyFee == 0) {
                break;
            }
            tempFeeToCollect += _divideWithRoundUp(tempYearlyFee, divisibilityFactor * 365);
        }
        return tempFeeToCollect - feeToCollect[_payer];
    }

    function _updateUserFee(address _address, uint _addedFee, uint _daysToCollect) internal {
        if (_daysToCollect == 0) {
            return;
        }

        lastTimeFeeToCollect[_address] += _daysToCollect * 1 days;
        LastTimeFeeToCollectUpdated(_address, lastTimeFeeToCollect[_address]);

        //emit event only if addedFee changed
        if (_addedFee == 0) {
            return;
        }
        feeToCollect[_address] += _addedFee;
        FeeToCollectUpdated(_address, _addedFee, feeToCollect[_address], _daysToCollect, balances[_address]);
    }

    function _divideWithRoundUp(uint _numerator, uint _denominator) internal constant returns(uint) {
        return _numerator / _denominator + (_numerator % _denominator == 0 ? 0 : 1);
    }
    ...
    function _transferACcoins(address _from, address _to, uint _value) internal {
        ...
        for (uint i = userRGAssetOwnershipCardsIndex[_from]; i < userRGAssetOwnershipCards[_from].length; i++) {
            RGAssetOwnershipCardInterface rgAssetOwnershipCard = userRGAssetOwnershipCards[_from][i];
    ...
    function getUserBalancesInAssetCards(address _owner) constant returns(bytes32[4][]) {
        ...
        for (uint i = userRGAssetOwnershipCardsIndex[_owner]; i < userRGAssetOwnershipCards[_owner].length; i++) {
            RGAssetOwnershipCardInterface rgAssetOwnershipCard = userRGAssetOwnershipCards[_owner][i];
            assetCards[k] = [
    ...
    function redemptionCancel(string _invoice) notUnderMigration() onlyOwner() returns(bool) {
        address invoice = address(keccak256(_invoice));
        uint invoiceBalance = balances[invoice];
    ...
    function _addToUserCards(address _user, RGAssetOwnershipCardInterface _rgAssetOwnershipCard) internal {
    ...
    function _removeFromUserCards(address _from, RGAssetOwnershipCard _rgAssetOwnershipCard) internal {
        ...
        if (fromACArrayLength > fromPositionInRGACArray) {
            RGAssetOwnershipCardInterface lastCard = userRGAssetOwnershipCards[_from][fromACArrayLength - 1];
    ...
    function redemptionSwap(address _rgAssetCard, string _invoice, address _rgHotWallet) onlyOwner() notUnderMigration() returns(bool) {
        ...
        _transferBalances(_rgHotWallet, invoice, balance);
    ...
    function setDefaultFeeTime(uint _feeTime) onlyOwner() notUnderMigration() returns(bool) {
        if (_feeTime == 0) {
            Error('Not allowed to set 0 fee time');
            return false;
        }

        //Not allowed to set _feeTime in the past
        //Not allowed to update _feeTime if defaultFeeTime already set and in the past
        if (_feeTime < now || (defaultFeeTime < now && defaultFeeTime != 0)) {
            Error('Not allowed fee time');
            return false;
        }

        defaultFeeTime = _feeTime;
        DefaultFeeTimeSet(defaultFeeTime);
        return true;
    }

    function updateDefaultFeeTimeSameDay(uint _feeTime) onlyOwner() notUnderMigration() returns(bool) {
        if (_feeTime == 0) {
            Error('Not allowed to set 0 fee time');
            return false;
        }
        ...
        if (defaultFeeTime == 0) {
            Error('Default fee time should be set');
            return false;
        }

        uint defaultFeeTimeRounded = defaultFeeTime / 1 days * 1 days;

        //Allowed to reset defaultFeeTime in range of current defaultFee day hours in the past
        if (!(defaultFeeTimeRounded < _feeTime && defaultFeeTimeRounded + 86399 >= _feeTime)) {
            Error('Update allowed for same day only');
            return false;
        }
    ...
    function setFeeCollectorAddress(address _feeCollector) onlyOwner() notUnderMigration() returns(bool) {
        if (_feeCollector == 0x0) {
            Error('Collector address is not valid');
            return false;
        }

        if (feeToCollect[_feeCollector] != 0) {
            Error('Collector address has fee debt');
            return false;
        }

        if (feeCollector != 0x0) {
            _removeNonFeeAddress(feeCollector);
        }

        feeCollector = _feeCollector;
        FeeCollectorSet(_feeCollector);
        _addNonFeeAddress(_feeCollector);
        return true;
    }

    function addNonFeeAddress(address _nonFeeAddress) onlyOwner() notUnderMigration() returns(bool) {
        if (_nonFeeAddress == 0x0) {
            Error('Non fee address is not valid');
            return false;
        }

        if (nonFeeAddresses[_nonFeeAddress]) {
            Error('Non fee address already added');
            return false;
        }

        if (feeToCollect[_nonFeeAddress] != 0) {
            Error('Non fee address has fee debt');
            return false;
        }

        _addNonFeeAddress(_nonFeeAddress);
        return true;
    }

    function _addNonFeeAddress(address _nonFeeAddress) internal {
        nonFeeAddresses[_nonFeeAddress] = true;
        NonFeeAddressAdded(_nonFeeAddress);
    }

    function removeNonFeeAddress(address _nonFeeAddress) onlyOwner() notUnderMigration() returns(bool) {
        if (!nonFeeAddresses[_nonFeeAddress]) {
            Error('Is not a nonFeeAddress');
            return false;
        }

        if (_nonFeeAddress == feeCollector) {
            Error('Not possible to remove collector');
            return false;
        }

        _removeNonFeeAddress(_nonFeeAddress);
        return true;
    }

    function _removeNonFeeAddress(address _nonFeeAddress) internal {

        //Reset the date to avoid fee calculation during the period in nonFeeAddresses list
        lastTimeFeeToCollect[_nonFeeAddress] = now / 1 days * 1 days;
        LastTimeFeeToCollectUpdated(_nonFeeAddress, lastTimeFeeToCollect[_nonFeeAddress]);

        nonFeeAddresses[_nonFeeAddress] = false;
        NonFeeAddressRemoved(_nonFeeAddress);
    }

    //Fee should be set with divisibility factor: fee * 10 ** 4
    //For instance, 0.02 % yearly fee should be set 2, equal to (0.0002 * 10000)
    //1 % yearly fee should be set 100, equal to (0.01 * 10000)
    function setYearlyFee(uint _fee) onlyOwner() notUnderMigration() returns(bool) {
        if (_fee > 1000) {
            Error('Set fee in range 0% - 10%');
            return false;
        }
    ...
    function collectFee(address _from) notUnderMigration() returns(bool) {
         _updateFee(_from);
        uint fee = feeToCollect[_from];
        if (fee == 0) {
        ...
        if (feeCollector == 0x0) {
            Error('Set feeCollector address first');
            return false;
        }

        if (balances[_from] < fee) {
            Error('Not enough balance to pay fee');
            return false;
        }

        _transferACcoins(_from, feeCollector, fee);

        _transferBalances(_from, feeCollector, fee);
        FeePayed(_from, fee);
        feeToCollect[_from] = 0;
        return true;
    }

    function _transferBalances(address _from, address _to, uint _value) internal {
