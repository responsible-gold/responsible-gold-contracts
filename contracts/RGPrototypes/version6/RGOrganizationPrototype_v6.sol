...
contract RGOrganizationFactoryInterface {
...
contract RGAccountInterface {
    ...
    function forward(address _to, uint _value, bytes _data, bool _revertOnFailedCall) returns(bool);
}
...
contract RGOrganizationPrototype_v6 {
    ...
    modifier onlySigned() {
        ...
        (success, isLastConsumption) = RGRuleAuthorizerInterface(ruleAuthorizer).consumeOperation(data, internalId);
        if (!success) {
    ...
    function RGOrganizationPrototype_v6() {
        constructOrganization(0x1, 0x1, 0x1);
    }

    function constructOrganization(address _organizationFactory, address _ruleAuthorizer, address _user) returns(bool) {
