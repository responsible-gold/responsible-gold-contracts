...
import {RGAssetOwnershipCardPrototype_v3 as RGAssetOwnershipCard} from './RGAssetOwnershipCardPrototype_v3.sol';
 ...
 * didn't happen yet.
 */
contract RGManagerPrototype_v3 is ERC20, OwnedPrototype {
    ...
    event InvoiceSwapped(string invoice, address invoiceAddress, uint amount);
    ...
    function RGManagerPrototype_v3() {
     ...
     * @return success.
     */
    function transfer(address _to, uint _value) notUnderMigration() notInvoice(_to) returns(bool success) {
     ...
     * @return success.
     */
    function transferFrom(address _from, address _to, uint _value) notUnderMigration() notInvoice(_to) returns(bool success) {
     ...
     *
     */
    function callbackTransfer(address _from, address _to, uint _value) notUnderMigration() notInvoice(_to) returns(bool success) {
        return _callbackTransfer(_from, _to, _value, msg.sender);
    ...
    function getUserBalancesInAssetCards(address _user) constant returns(bytes32[4][]) {
        bytes32[4][] memory assetCards = new bytes32[4][](userRGAssetOwnershipCards[_user].length);

        uint k = 0;
        for (uint i = userRGAssetOwnershipCardsIndex[_user]; i < userRGAssetOwnershipCards[_user].length; i++) {
            RGAssetOwnershipCard rgAssetOwnershipCard = userRGAssetOwnershipCards[_user][i];
            assetCards[k] = [
                bytes32(address(rgAssetOwnershipCard)),
                bytes32(rgAssetOwnershipCard.totalSupply()),
                bytes32(rgAssetOwnershipCard.balanceOf(_user)),
                bytes32(rgAssetOwnershipCard.balanceOf(_user) == rgAssetOwnershipCard.totalSupply() ? 1 : 0)
            ];
    ...
    function redemptionSwap(string _chip, uint _totalCoins, string _invoice) onlyOwner() notUnderMigration() onlyUnique(_chip) returns(bool) {
        address invoice = address(keccak256(_invoice));
        ...
        if (balance != _totalCoins) {
            Error('Locked coins != to new asset');
            return false;
        }

        if (!_transfer(invoice, contractOwner, balance)) {
            return false;
        }

        _deploy(_chip, _totalCoins, invoice);
        InvoiceSwapped(_invoice, invoice, balance);
        return true;
    }
