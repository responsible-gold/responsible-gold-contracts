...
import {RGAssetOwnershipCardPrototype_v1 as RGAssetOwnershipCard} from './RGAssetOwnershipCardPrototype_v1.sol';
 ...
 * didn't happen yet.
 */
contract RGManagerPrototype_v1 is ERC20, OwnedPrototype {
    ...
    event Deployed(address rgAssetOwnershipCardAddress, bytes32 chip, uint coins);
    ...
    modifier onlyUnique(bytes32 _chip) {
        if (rGAssetOwnershipCardChips[_chip] != 0x0) {
    ...
    function RGManagerPrototype_v1() {
     ...
     * @return success.
     */
    function deploy(bytes32 _chip, uint _totalCoins, address _coinsOwner) onlyOwner() onlyUnique(_chip) returns(bool) {
     ...
     * @return success.
     */
    function deployWithoutGcoins(bytes32 _chip, uint _totalCoins, address _assetCardOwner) onlyOwner() onlyUnique(_chip) returns(bool) {
        _deployRGAssetCard(_chip, _totalCoins, _assetCardOwner);
        return true;
    }

    function _deployRGAssetCard(bytes32 _chip, uint _totalCoins, address _coinsOwner) internal returns(RGAssetOwnershipCard) {
        ...
        rGAssetOwnershipCardChips[_chip] = address(rgAssetOwnershipCard);
     ...
     * @return success.
     */
    function mintGcoins(bytes32 _chip, address _gCoinsOwner) onlyOwner() returns(bool) {
        address rgAssetOwnershipCardAddress = rGAssetOwnershipCardChips[_chip];
     ...
     *
     */
    function callbackTransfer(address _from, address _to, uint _value) returns(bool success) {
